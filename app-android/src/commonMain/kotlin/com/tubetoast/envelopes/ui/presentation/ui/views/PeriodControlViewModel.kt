...
class PeriodControlViewModel(
    ...
    private val filterByYearFlow = settingsRepository.getSettingFlow(Setting.Key.FILTER_BY_YEAR)
    private val filterByYear get() = filterByYearFlow.value.checked
    ...
    fun changePeriodType() {
        settingsRepository.saveChanges(
            filterByYearFlow.value.run { copy(checked = !checked) }
        )
    }

    fun nextPeriod() =
        selectedPeriodRepository.run {
            val today = today()
