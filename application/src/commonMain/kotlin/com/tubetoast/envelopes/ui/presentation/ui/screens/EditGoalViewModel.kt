        ...
        goalId?.let { id ->
            viewModelScope.launch {
                goalInteractor.getExactGoal(id.id())?.let { foundGoal ->
                    val foundCategories =
                        goalSnapshotInteractor.goalSnapshots.value
                            .find { it.goal == foundGoal }
                            ?.categories
...
//        reset() // for no flicker on confirm
    ...
    private fun update(update: Goal.() -> Goal = { this }) {
...
private class CreateGoalMode(
    ...
    override suspend fun canConfirm(
        goal: Goal?,
        categories: Set<Category>
    ) = goal?.run {
...
private class EditGoalMode(
    ...
    override suspend fun canConfirm(
        ...
        isNotEmpty() &&
            hasValidDateRange() &&
            (this != editedGoal || foundCategories != categories) &&
            notSameNameAsOtherExisting() &&
            categories.isNotEmpty()
    } ?: false

    private suspend fun Goal.notSameNameAsOtherExisting() = name == editedGoal.name || goalInteractor.getGoalByName(name) == null
