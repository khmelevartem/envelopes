...
class EditCategoryViewModel(
    ...
    abstract class Mode(
        ...
        abstract fun envelope(
            id: Id<Envelope>?,
            change: (Envelope) -> Unit
        )
    ...
    fun setName(input: String) {
        updateCategory(draftCategory.value.copy(name = input))
    }

    fun setLimit(input: String) {
        updateCategory(draftCategory.value.copy(limit = Amount(input.toLongOrNull() ?: 0)))
    }

    fun confirm() {
        viewModelScope.launch {
            mode.confirm(draftCategory.value, _envelope.value)
            reset()
        }
    }
