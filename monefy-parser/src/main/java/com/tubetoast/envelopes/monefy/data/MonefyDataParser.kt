...
class MonefyDataParser(
    ...
    private val snapshots: MutableMap<String, MutableSet<Transaction>> = mutableMapOf()

    fun parse(): List<CategorySnapshot> {
        file.reader(Charsets.UTF_8).use { reader ->
            val lines = reader.readLines()
            val columns = MonefyDataColumns.parse(lines.first())
            lines.subList(1, lines.size).forEach { line ->
                line.process(columns)
            }
    ...
    private fun String.process(columns: MonefyDataColumns) {
