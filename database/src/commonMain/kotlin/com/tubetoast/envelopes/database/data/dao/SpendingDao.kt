...
abstract class SpendingDao : StandardDao<SpendingEntity> {
    ...
    abstract override suspend fun getAll(): List<SpendingEntity>

    @Query("SELECT * from spendingentity WHERE foreignKey LIKE :foreignKey")
    abstract override suspend fun getCollection(foreignKey: Int): List<SpendingEntity>

    @Query("SELECT * from spendingentity WHERE valueId LIKE :valueId")
    abstract override suspend fun get(valueId: Int): SpendingEntity?

    @Query("SELECT * from spendingentity WHERE primaryKey LIKE :primaryKey")
    abstract override suspend fun getByKey(primaryKey: Int): SpendingEntity?

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract override suspend fun write(databaseEntity: SpendingEntity)
    ...
    @Update
    abstract override suspend fun update(newValue: SpendingEntity): Int
}
