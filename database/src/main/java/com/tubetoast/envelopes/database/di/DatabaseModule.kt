...
fun databaseModule(context: Context) = module {
    ...
    single<UpdatingEnvelopesRepository>() {
        EnvelopesRepositoryDatabaseBase(EnvelopeDataSource(db.envelopeDao()))
    }
    single<UpdatingCategoriesRepository>() {
        CategoriesRepositoryDatabaseBase(CategoryDataSource(db.categoryDao()))
    }
    single<UpdatingSpendingRepository>() {
        SpendingRepositoryDatabaseBase(SpendingDataSource(db.spendingDao()))
    }
}